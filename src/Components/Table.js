...
function Table() {
	...
	const reveiwsPerPage = 10;
	

	console.log(reviews);
	
	useEffect(() => {
		const referdb = ref(database, 'reviews');  // To refer database: dbname=> reviews
		const unsubscribe = onValue(referdb, (snapshot) => {
			const data = snapshot.val();  // pass values on snapshot
			if (data) {
				const reviewsArray = Object.entries(data)   //Create constatnt array
					.map(([id, value]) => ({
						id,
						...value,
					}))
					.sort((a,b) => b.timestamp - a.timestamp); //Sort to latest review come first
	...
	// const indexOfFirstReview = indexOfLastReview - reveiwsPerPage;
	// const indexOfLastReview = currentPage * reveiwsPerPage;
				...
				<table class="w-full rounded-xl">
					<thead>
						<tr class="bg-gray-50">
							...
							<th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize"> Name </th>
							<th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize"> Rating  </th>
							<th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize min-w-[150px]"> Comment </th>
